...
import {
  ...
  TextInput, Alert, KeyboardAvoidingView, Platform, PanResponder, Image,
} from 'react-native';
...
async function updatePlanTemplate(planId, exercises) {
  const { error } = await supabase.from('workout_plans').update({ template_exercises: exercises }).eq('id', planId);
  if (error) throw error;
}
...
// ─── Template exercise editor sub-screen ──────────────────────────────────────
function TemplateEditor({ plan, allSessions, onSave, onBack, colors }) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.bg }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        {exercises.map((ex, idx) => {
          ...
          return (
            <View key={idx} style={{
              ...
              {imgUrl && (
                <TouchableOpacity onPress={() => Alert.alert(ex, '', [{ text: 'Close' }])} style={{ paddingHorizontal: 8 }}>
      ...
      {suggestions.length > 0 && (
        <View style={{ backgroundColor: colors.surface, borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.border, maxHeight: 200 }}>
          <ScrollView keyboardShouldPersistTaps="handled">
            {suggestions.map(n => (
              <TouchableOpacity key={n} style={{ paddingHorizontal: 16, paddingVertical: 11, borderBottomWidth: 1, borderBottomColor: colors.border }}
                onPress={() => setNewEx(n)}>
      ...
      <TouchableOpacity onPress={() => onSave(exercises.map(name => ({ name })))}
...
export default function WorkoutPlansScreen({ navigation, route }) {
  ...
  const [templatePlan, setTemplatePlan] = useState(null); // non-null = show template editor
  ...
  const savePlanTemplateMut = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries(planKey),
  });
  ...
  // If template editor active, show it
  if (templatePlan) {
    return (
      <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: colors.bg }}>
        <ScreenHeader title={templatePlan.name} onBack={() => setTemplatePlan(null)} />
        <TemplateEditor
          plan={templatePlan}
          allSessions={allSessions}
          colors={colors}
          onSave={(exercises) => {
            savePlanTemplateMut.mutate({ planId: templatePlan.id, exercises });
            setTemplatePlan(null);
          }}
          onBack={() => setTemplatePlan(null)}
        />
      </SafeAreaView>
    );
  }
            ...
            {orderedPlans.map((plan, idx) => {
              ...
              return (
                <View key={plan.id} style={{
                  ...
                  <TouchableOpacity onPress={() => setTemplatePlan(plan)} style={{ padding: 7 }}>
